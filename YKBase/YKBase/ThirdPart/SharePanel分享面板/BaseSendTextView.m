...
- (instancetype)initWithFrame:(CGRect)frame andButtonTitleAndImages:(NSArray *)titleAndImages withColumnNums:(NSInteger)columnNum withBlock:(BaseSendTextBlock)block{
    ...
    if (self) {
        ...
        if (titleAndImages.count > 0) {
            ...
            _btnFrameY = frame.size.height / 1.40 - _btnViewSize.height * _btnRowNum / 2.0;
            _btnItemSpace = _btnFrameX;
            for (int i = 0; i < titleAndImages.count; i++) {
                SharePanelModel * model = [titleAndImages objectAtIndex:i];
